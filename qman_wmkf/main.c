...
{	
	...
	if (argc >= 2)
	{
		strncpy(tmp_line, (const char *)*++argv, MAX_ARG_LENGTH);
	...
	qinit(&qu[0], (size_t *)&mem_space[0][0], qu_size[0]);

	qinit(&qu[1], (size_t *)&mem_space[1][0], qu_size[1]);

	qinit(&qu[2], (size_t *)&mem_space[2][0], qu_size[2]);

	/*******************************************************
	 *** Read the queue info and data if the file exists ***
	 *******************************************************/
	FILE * quf;
	quf = fopen("./queues.bin", "rb+");
	if (NULL != quf)
	{
		get_queues(&qu[0], &qu[1], &qu[2], quf);
		fclose(quf);
	}
	...
	if ((int)cmd_push == command)
		qpush(&qu[argument[0] - 1], argument[1]);
	...
	if ((int)cmd_merge == command)
		qmerge( &qu[2], &qu[0], &qu[1], (size_t *)&mem_space[2][0], QUEUE3_LENGTH);
