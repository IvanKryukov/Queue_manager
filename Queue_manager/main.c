...
int main(int argc, char * argv)
{
	...
	uint32_t i = 1;
	memset((void*)&mem_space[0][0], 0, sizeof(mem_space));
	...
	while (1)
	{
		...
		if ( 'i' == board_char_0)
		{
			...
			{
				const e_qres qres = qinit(&qu[qu_count], &mem_space[qu_count][0], qu_size[qu_count]);
				print_qret(qres);
		...
		if ('m' == board_char_0)
		{
			const e_qres qres = qmerge( &qu[2], &qu[0], &qu[1], &mem_space[2][0], QUEUE3_LENGTH);
			print_qret(qres);
